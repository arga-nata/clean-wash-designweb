...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_order'])) {
  ...
  if (!$conn) {
    ...
    $name = ucwords(strtolower($_POST['custName']));
    $phone = $_POST['custPhone'];
    $address = ucwords(strtolower($_POST['custAddress']));
...
<section class="order-section" style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          ...
          <div class="customer-grid">
            ...
            <div style="display: flex; gap: 20px; align-items: flex-start; flex-wrap: wrap;">
              ...
              <div style="flex: 1; min-width: 200px; margin-top: 10px">
                <label class="fw-bold" style="font-size: 0.9rem; display: block; margin-bottom: 10px;">Area
                  Pengambilan:</label>
  ...
  if (myForm) {
    myForm.onsubmit = async function (e) {
      ...
      try {
        ...
        alert("Ter terjadi kesalahan, silakan coba lagi.");
        btn.innerText = originalText;
        btn.style.backgroundColor = "";
        btn.disabled = false;
      }
